...
using KoalaKit.Cosmetics.Models;
...
namespace Koala.Authentications.Jwt
{
    public class JwtKoalaSigninService : IKoalaSigninService
    {
        ...
        public async Task<KoalaSignInResult> ValidateAndSignIn(SignInParameters parameters)
        {
        ...
        public async Task<KoalaSignInResult> ValidateRefreshToken(string refreshToken)
        {
        ...
        public async Task<KoalaSignInResult> ValidateRegistrationToken(string registrationToken)
        {
            var data = tokenizationService.GetData<KoalaRegistrationValidationParameters>(registrationToken);
            if (data == null)
            {
                return new KoalaSignInResult("registration-token");
            }
            var result = await SignIn(data.Id);
            return result;
        }
    }
}
